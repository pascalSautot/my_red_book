...
BEGIN_APP_DECLARATION(InstancingExample)
    ...
    enum Attrib_IDs { vPosition=0, vNormal=1, vTexture=2, vWeight = 3,  vColor=4 };
END_APP_DECLARATION()
...
void InstancingExample::Initialize(const char * title)
{
    ...
    ShaderInfo  shaders[] =
    {
        { GL_VERTEX_SHADER, "media/shaders/instancing/instancing.vert" },
        { GL_FRAGMENT_SHADER, "media/shaders/instancing/instancing.frag" },
        { GL_NONE, NULL }
    };
   ...
   // "model_matrix" is actually an array of 4 matrices
    ...
    // Load the object
    object.LoadFromVBM("media/armadillo_low.vbm", vPosition, vNormal, vTexture);
    ...
    glVertexAttribDivisor(vWeight, 1);
    // It's otherwise the same as any other vertex attribute - set the pointer and enable it
    glVertexAttribPointer(vWeight, 4, GL_FLOAT, GL_FALSE, 0, NULL);
    glEnableVertexAttribArray(vWeight);
