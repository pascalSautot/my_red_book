...
typedef struct VBM_BOUNDING_BOX_t
{
    float min[3];
    float max[3];
    float l_max,l_min;
...
public:
    ...
    virtual void Render(unsigned int frame_index = 0, unsigned int instances = 0)=0;    
...
public:
    ...
    void Build(GLfloat **vertices, GLfloat **colors, GLfloat **normals,const GLint count);

    virtual void Render(unsigned int frame_index = 0, unsigned int instances = 1);    
    static GLint m_dim;
    enum { vertexIndex, colorIndex,normalIndex,numIndex};
protected:
    GLuint m_first;
    GLuint m_count;
    // alloc m_count elements
    void Destroy(void);
    GLfloat **m_data[3];
};
class VBObject : public BaseObject
{
public:
    VBObject(void);
    ...
    const VBM_BOUNDING_BOX & BoundingBox(void)const {return m_bb;}
protected:
